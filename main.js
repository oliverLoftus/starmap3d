...
function drawBackground(bg) {
  const cx = perspective_X(bg) + zero_X;
  const cy = perspective_Y(bg) + zero_Y;
  const size = perspective_R(bg) * 2; // Use perspective-based scaling

  const image = document.createElementNS("http://www.w3.org/2000/svg", "image");
  image.setAttribute("href", "images/background.jpg"); // Path to your bitmap image
  image.setAttribute("x", cx - size / 2); // Center the image
  image.setAttribute("y", cy - size / 2);
